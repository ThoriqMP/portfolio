...
<div class="space-y-6 max-w-3xl">
    ...
    <div class="pb-6 border-b-4 border-black">
        ...
        <h1 class="text-4xl font-black uppercase tracking-tighter text-black">Tambah Riwayat Pendidikan</h1>
        <p class="text-xs font-bold uppercase tracking-wider text-slate-500">// Lengkapilah form berikut untuk menambah riwayat sekolah/kuliah.</p>
    </div>

    <!-- Form Card (Brutalist Box) -->
    <div class="bg-white border-4 border-black p-6 sm:p-8 shadow-[6px_6px_0px_#000000]">
        <form action="{{ route('admin.educations.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Institution Name -->
            <div>
                <label for="institution_name" class="block text-xs font-black uppercase tracking-widest text-black mb-2">Nama Institusi</label>
                <input type="text" name="institution_name" id="institution_name" value="{{ old('institution_name') }}" placeholder="Contoh: Universitas Indonesia" required
            ...
            <div>
                ...
                <input type="text" name="degree" id="degree" value="{{ old('degree') }}" placeholder="Contoh: S1 Teknik Informatika" required
                    class="w-full bg-white border-3 border-black rounded-none px-4 py-3 text-black font-sans font-bold placeholder-slate-400 focus:outline-none focus:bg-orange-50/70 focus:-translate-y-0.5 focus:shadow-[4px_4px_0px_#000000] transition-all duration-150">
                @error('degree')
                    <p class="text-rose-600 text-xs font-bold uppercase tracking-wide mt-1.5">// {{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="start_year" id="start_year" value="{{ old('start_year', date('Y')) }}" min="1900" max="2100" required
                ...
                <div>
                    ...
                    <input type="number" name="end_year" id="end_year" value="{{ old('end_year') }}" min="1900" max="2100" placeholder="Kosongkan jika masih aktif belajar"
                        class="w-full bg-white border-3 border-black rounded-none px-4 py-3 text-black font-sans font-bold placeholder-slate-400 focus:outline-none focus:bg-orange-50/70 focus:-translate-y-0.5 focus:shadow-[4px_4px_0px_#000000] transition-all duration-150">
                    @error('end_year')
                        <p class="text-rose-600 text-xs font-bold uppercase tracking-wide mt-1.5">// {{ $message }}</p>
                    @enderror
            ...
            <!-- Description -->
            <div>
                <label class="block text-xs font-black uppercase tracking-widest text-black mb-2">Pencapaian / Deskripsi (Opsional)</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addPointRow(value = '') {
        ...
        div.innerHTML = `
            ...
            <input type="text" value="${escapeHtml(value)}" placeholder="Tuliskan pencapaian akademik, kepanitiaan, atau detail..." 
    ...
    function updateTextareaValue() {
        if (currentFormat === 'list') {
            ...
            textarea.value = points.length > 0 ? JSON.stringify(points) : '';
        }
    }
