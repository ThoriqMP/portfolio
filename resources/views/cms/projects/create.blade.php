...
<div class="space-y-6 max-w-3xl">
    ...
    <div class="pb-6 border-b-4 border-black">
        ...
        <h1 class="text-4xl font-black uppercase tracking-tighter text-black">Tambah Proyek Baru</h1>
        <p class="text-xs font-bold uppercase tracking-wider text-slate-500">// Lengkapilah form berikut untuk menambah portofolio baru.</p>
    </div>
    ...
    <div class="bg-white border-4 border-black p-6 sm:p-8 shadow-[6px_6px_0px_#000000]">
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Masukkan nama atau judul proyek" required
            ...
            <!-- Project Image -->
            <div>
                <label for="image" class="block text-xs font-black uppercase tracking-widest text-black mb-2">Gambar Utama / Mockup (Thumbnail - Opsional)</label>
                ...
                <p class="text-slate-500 text-xxs font-bold uppercase mt-1.5">// Opsional pada pembuatan draf. Mendukung JPEG, JPG, PNG (Maks 10MB).</p>
                @error('image')
                    <p class="text-rose-600 text-xs font-bold uppercase tracking-wide mt-1.5">// {{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative w-full">
                    <select name="thumbnail_composition" id="thumbnail_composition" required
                        ...
                        <option value="single" {{ old('thumbnail_composition', 'single') == 'single' ? 'selected' : '' }}>Tunggal (1 Gambar Utama)</option>
                        <option value="split" {{ old('thumbnail_composition') == 'split' ? 'selected' : '' }}>Split Kolase (50/50 Dual Grid)</option>
                        <option value="mosaic" {{ old('thumbnail_composition') == 'mosaic' ? 'selected' : '' }}>Mosaik Kolase (2/3 & 1/3 Mosaic Grid)</option>
                        <option value="carousel" {{ old('thumbnail_composition') == 'carousel' ? 'selected' : '' }}>Slider Otomatis (Carousel Slideshow)</option>
                    </select>
            ...
            <div class="border-3 border-dashed border-black bg-orange-50/10 p-6 text-center space-y-4 shadow-[4px_4px_0px_#000000] hover:shadow-[6px_6px_0px_#000000] transition-all">
                ...
                <p class="text-xs font-bold text-slate-700 uppercase tracking-wide leading-relaxed max-w-lg mx-auto">
                    Kustomisasi galeri media, urutan rendering mockup, dan komposisi lebar grid gambar (asimetris) tersedia setelah draf proyek disimpan pertama kali.
                </p>
                <div>
                    <button type="submit" formaction="{{ route('admin.projects.storeDraftNew') }}"
            ...
            <div>
                ...
                <div class="relative w-full">
                    <select name="grid_span" id="grid_span" required
                        class="w-full bg-white border-3 border-black rounded-none px-4 py-3 text-black font-sans font-bold appearance-none focus:outline-none focus:bg-orange-50/70 focus:-translate-y-0.5 focus:shadow-[4px_4px_0px_#000000] transition-all duration-150 cursor-pointer">
                        <option value="1" {{ old('grid_span', 1) == 1 ? 'selected' : '' }}>Standar (1 Blok Kolom)</option>
                        <option value="2" {{ old('grid_span') == 2 ? 'selected' : '' }}>Lebar (2 Blok Kolom)</option>
                        <option value="3" {{ old('grid_span') == 3 ? 'selected' : '' }}>Penuh (3 Blok Kolom / Full Width)</option>
                    </select>
            ...
            <div>
                ...
                <input type="url" name="project_link" id="project_link" value="{{ old('project_link') }}" placeholder="https://github.com/username/project-name"
                    class="w-full bg-white border-3 border-black rounded-none px-4 py-3 text-black font-sans font-bold placeholder-slate-400 focus:outline-none focus:bg-orange-50/70 focus:-translate-y-0.5 focus:shadow-[4px_4px_0px_#000000] transition-all duration-150">
                @error('project_link')
                    <p class="text-rose-600 text-xs font-bold uppercase tracking-wide mt-1.5">// {{ $message }}</p>
                @enderror
